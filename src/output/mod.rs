...
///
/// Writes `<out_dir>/buttons.<ext>` for every requested `file_types`
/// extension (`hpp`, `json`, `rs`, `zig`, `cs`). Used directly from
/// `main.rs` because the buttons set is finalised by the analysis
/// pass, but the disk emit is delayed until after signatures so all
/// SDK files land in the same `sdk/` directory in one batch.
pub fn write_buttons(
    ...
    for file_type in file_types {
        ...
        if file_type != "json" {
            writeln!(
                fmt,
                "// CS2 SDK — generated by cs2-universal-dumper v{}",
