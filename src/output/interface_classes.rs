...
pub fn render_hpp(classes: &[IfaceClass], build_number: Option<u32>) -> String {
    ...
    s.push_str("//\n");
    s.push_str("// Interfaces. Each struct's pure-virtuals are declared in vtable-slot order\n");
    s.push_str("// so its layout mirrors the game's. Cast a resolved interface pointer and\n");
    s.push_str("// call methods as ordinary virtuals — the compiler handles dispatch:\n");
    s.push_str("//\n");
    ...
    s.push_str("// only reserve the slot — fill them in as you reverse them.\n");
    s.push_str("\n#pragma once\n");
